...
describe("markdownAdapter — parser selection", () => {
  it("passes the configured Remark parser through the Astro wrapper", () => {
    const parseMarkdown = vi.spyOn(remarkMarkdownParser, "parseMarkdown");
    markdownAdapter.parse("# Hello\n", "docs/sample.md", { markdownParser: "remark" });
    expect(parseMarkdown).toHaveBeenCalledOnce();
    parseMarkdown.mockRestore();
  });
});
