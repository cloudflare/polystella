...
describe("parseMarkdown", () => {
  ...
  it("falls back to Remark when Satteri cannot parse MDX", async () => {
    vi.resetModules();
    vi.doMock("satteri", () => ({
      markdownToMdast: vi.fn(),
      mdxToMdast: vi.fn(() => {
    ...
    try {
      const { parseMdx } = await import("../../src/parsing/parse.js");
      const ast = parseMdx("<Callout>Fallback</Callout>\n");
      expect(ast.children[0]).toMatchObject({
        type: "paragraph",
        children: [{ type: "mdxJsxTextElement" }],
      });
    } finally {
      vi.doUnmock("satteri");
    }
