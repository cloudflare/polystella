...
import type { Logger, Translator } from "@cloudflare/polystella-core";
...
describe("runTranslationPass — staging output", () => {
  ...
  it("rewrites source-relative MDX ESM imports only in staged bytes", async () => {
    ...
    expect(cachedBody).toContain("aiTranslated: true");
    expect(cachedBody).toContain("aiTranslationModel: stub/mdx-imports-1");
    expect(cachedBody).toMatch(/aiTranslatedAt: \d{4}-\d{2}-\d{2}T/);
