...
export function createStructuredAstroAdapter<TParsed>(
  adapter: FileAdapter<TParsed>,
  peekNoTranslate: (parsed: TParsed) => boolean,
  ...
  return {
    ...
    selectedValuesForHash(parsed, _source, options) {
      const selected: Record<string, unknown> = {};
      for (const keyPath of resolveConcretePaths({
        parsed,
        sourcePath: options.sourcePath,
        translatableKeys: options.translatableKeys,
      })) {
        const value = readAtPath(parsed, parsePath(keyPath).segments as PathSegment[]);
        if (value !== undefined) selected[keyPath] = value;
      }
      return selected;
    },
    peekNoTranslate,
    rewriteUrls(bytes, options) {
      if (options.paths.length === 0) return bytes;
      const parsed = adapter.parse(bytes);
      const translations = new Map<string, string>();
      for (const rule of options.paths) {
        for (const keyPath of expandPath(rule, parsed)) {
          const value = readAtPath(parsed, parsePath(keyPath).segments as PathSegment[]);
          if (typeof value !== "string") continue;
          const rewritten = options.rewriter(value);
          if (rewritten !== null && rewritten !== value) translations.set(keyPath, rewritten);
        }
      }
      return translations.size === 0 ? bytes : adapter.applyTranslations(parsed, bytes, translations);
    },
  };
}
