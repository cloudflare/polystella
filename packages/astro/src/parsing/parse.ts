...
const satteriMarkdownParser: PortableMarkdownParser = {
  ...
  parseMdx(source) {
    try {
      ...
      return remarkMarkdownParser.parseMdx(source);
    }
  },
};

export function resolveMarkdownParser(parser: MarkdownParser = DEFAULT_MARKDOWN_PARSER): PortableMarkdownParser {
  return parser === "satteri" ? satteriMarkdownParser : remarkMarkdownParser;
}

/** Pure: no I/O, no Astro coupling. */
export function parseMarkdown(source: string, opts: MarkdownParseOptions = {}): Root {
  return resolveMarkdownParser(opts.parser).parseMarkdown(source);
}
...
export function parseMdx(source: string, opts: MarkdownParseOptions = {}): Root {
  return resolveMarkdownParser(opts.parser).parseMdx(source);
}
