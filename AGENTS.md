...
| :-------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm test`           | Run all app, package, Astro, workerd, and boundary tests.                                                                                              |
| `pnpm test:watch`     | Run the Astro package tests in watch mode.                                                                                                             |
| `pnpm build`          | Build all four public packages. Astro and EmDash emit standalone CLI entries.                                                                          |
| `pnpm typecheck`      | Build all four public packages, then typecheck every public package and private app against its package-local `tsconfig.json`.                         |
...
| :----------------------------------------------------- | :---------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| Add a file-format adapter                              | `packages/core/src/adapters/<name>.ts`; wrap/register in `packages/astro/src/parsing/`                | `FileAdapter` in `packages/core/src/adapters/adapter.ts`                                                              | [#adapter-contract](./ARCHITECTURE.md#adapter-contract); recipe in [contributor SKILL](./skills/polystella-contributor/SKILL.md#add-adapter)        |
| Add a shared catalog CLI subcommand                    | Handler in `packages/core/src/cli/<name>.ts`; register in `packages/core/src/cli/run-command.ts`      | Argv parser + `run<Name>(args, deps)`                                                                                 | Recipe in [contributor SKILL](./skills/polystella-contributor/SKILL.md#add-cli-subcommand)                                                          |
| Add a translation provider                             | Factory in `packages/core/src/providers/`; map config in `packages/astro/src/translation/provider.ts` | `Translator` in `packages/core/src/translator.ts`                                                                     | [#translator-contract](./ARCHITECTURE.md#translator-contract); recipe in [contributor SKILL](./skills/polystella-contributor/SKILL.md#add-provider) |
...
| Edit UI-string handling                                | `packages/core/src/catalog/translate.ts`, `packages/core/src/cli/{drift,sync,translate-ui}.ts`        | Three drift modes; layout-aware writer; `{{token}}` preservation                                                      | [#ui-strings](./ARCHITECTURE.md#ui-strings)                                                                                                         |
| Edit catalog-only adoption                             | `packages/core/src/catalog/index.ts`, `packages/astro/src/catalog/*`                                  | Pure imports; middleware binds only `t` + `lhref`                                                                     | [#ui-strings](./ARCHITECTURE.md#ui-strings); [consumer SKILL](./skills/polystella-consumer/SKILL.md#catalog-only-adoption)                          |
| Modify the EmDash plugin                               | `packages/emdash/src/*`                                                                               | Provider/locale/model bounds stay server-owned; collection policy is administrator-owned; Git catalogs stay canonical | [#package-boundaries](./ARCHITECTURE.md#package-boundaries)                                                                                         |
| Modify the hosted Translation Agent                    | `apps/translation-agent/{api,ui}/*`                                                                   | Existing HTTP/UI contracts stay compatible; private resources remain remote; production deployment stays separate     | [`TRANSLATION_AGENT_MIGRATION.md`](./TRANSLATION_AGENT_MIGRATION.md)                                                                                |
...
low-level contracts from core, formats from the `@cloudflare/polystella-core/adapters`
subpath, and transports from the `@cloudflare/polystella-core/providers`
subpaths; do not add compatibility shims to the Astro package.
...
- Add a Changesets entry for every change that affects the published
  ...
  maintenance).
- Let Changesets version core and EmDash independently while
