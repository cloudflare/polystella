...
| `pnpm build`          | Build all seven public packages. Astro and EmDash emit standalone CLI entries.                                                                         |
| `pnpm typecheck`      | Build all seven packages, then typecheck every public package against its package-local `tsconfig.json`.                                               |
| `pnpm check:packages` | Pack all seven public packages and exercise every export from a clean temporary consumer.                                                              |
...
| Add a shared catalog CLI subcommand                    | Handler in `packages/cli/src/<name>.ts`; register in `packages/cli/src/run-command.ts`           | Argv parser + `run<Name>(args, deps)`                                                                                 | Recipe in [contributor SKILL](./skills/polystella-contributor/SKILL.md#add-cli-subcommand)                                                          |
...
| Edit UI-string handling                                | `packages/core/src/catalog/translate.ts`, `packages/cli/src/{drift,sync,translate-ui}.ts`        | Three drift modes; layout-aware writer; `{{token}}` preservation                                                      | [#ui-strings](./ARCHITECTURE.md#ui-strings)                                                                                                         |
