...
export type ButtonVariant = "primary" | "secondary" | "ghost" | "destructive" | "secondary-destructive" | "outline";
...
export const buttonVariantClasses: Record<ButtonVariant, string> = {
  ...
  secondary: "bg-card text-foreground ring ring-border shadow-sm hover:bg-accent hover:ring-border-strong",
  ghost: "bg-transparent text-foreground shadow-none hover:bg-accent",
  destructive: "bg-danger text-white shadow-sm hover:bg-danger/90",
  "secondary-destructive": "bg-card text-danger ring ring-border shadow-sm hover:bg-accent hover:ring-danger/40",
...
export function buttonVariants({ variant = "secondary", size = "base", shape = "base" }: ButtonVariantsOptions = {}): string {
  // base + square inherit `rounded-lg` from buttonBase; circle overrides it
  // to a full pill.
  const dims = shape === "base" ? buttonSizeText[size] : cn(buttonSizeCompact[size], "p-0", shape === "circle" && "rounded-full");
