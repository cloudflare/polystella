...
function initToc(root: HTMLElement): () => void {
  ...
  const activeRailPath = activePath;

  const scrollHost = root.closest<HTMLElement>("[data-nb-toc-scroll-host]") ?? root;
  const slugs = Array.from(links)
    .map((link) => link.dataset.nbSlug)
    .filter((slug): slug is string => typeof slug === "string" && slug.length > 0);
  if (slugs.length !== links.length) return () => {};

  const headingEls = slugs.map((slug) => document.getElementById(slug)).filter((el): el is HTMLElement => el instanceof HTMLElement);
  ...
  function buildRail() {
    ...
    const measure = (subPath: string) => {
      activeRailPath.setAttribute("d", subPath);
      return activeRailPath.getTotalLength();
    };
    ...
    activeRailPath.setAttribute("d", d);
  ...
  function applyActive(index: number, instant: boolean) {
    ...
    if (instant) {
      activeRailPath.setAttribute("data-initial", "true");
      // Force recalc so only opacity transitions on first paint (no dash sweep).
      void activeRailPath.getBoundingClientRect();
    }

    activeRailPath.style.strokeDasharray = `${seg.length} ${totalLength + 1}`;
    activeRailPath.style.strokeDashoffset = `${-seg.start}`;

    if (instant) {
      requestAnimationFrame(() => {
        activeRailPath.setAttribute("data-ready", "true");
        requestAnimationFrame(() => {
          activeRailPath.removeAttribute("data-initial");
        });
      });
    }
  }
  ...
  function releaseStalePin() {
    ...
    const pinnedSlug = slugs[pinnedIndex];
    const heading = pinnedSlug ? document.getElementById(pinnedSlug) : null;
    if (!heading) {
      pinnedIndex = null;
  ...
  navElement.addEventListener(
    ...
    (e) => {
      ...
      if (!link) return;
      const linkSlug = link.dataset.nbSlug;
      if (!linkSlug) return;
      const i = slugs.indexOf(linkSlug);
      if (i === -1) return;
      pinnedIndex = i;
      const headingSlug = slugs[i];
      const heading = headingSlug ? document.getElementById(headingSlug) : null;
      const rect = heading?.getBoundingClientRect();
