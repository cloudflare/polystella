...
const defaultFilters: PagefindFilters | undefined =
  config.versions && config.versions.deprecated && config.versions.deprecated.length > 0 ? { status: { none: "deprecated" } } : undefined;
...
export const provider: SearchProvider = {
  ...
  async search(query) {
    ...
    const search = await pagefind.search(query, defaultFilters ? { filters: defaultFilters } : undefined);
