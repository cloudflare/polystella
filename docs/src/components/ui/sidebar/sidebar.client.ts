...
function initFilter(root: HTMLElement): (() => void) | null {
  ...
  const inputElement = input ?? root.parentElement?.querySelector<HTMLInputElement>("[data-nb-sidebar-filter-input]") ?? null;
  if (!inputElement) return null;
  const filterInput = inputElement;
  ...
  return () => {
    ...
    filterInput.removeEventListener("keydown", handleKeydown);
    resetFilter(root);
  };
}
...
function resetFilter(root: HTMLElement): void {
  ...
  root.querySelectorAll<HTMLElement>("[data-nb-sidebar-group][data-nb-opened-by-filter]").forEach((group) => {
